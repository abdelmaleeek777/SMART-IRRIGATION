...
function DetailRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-iceBlue bg-white/70 px-4 py-3 shadow-sm">
      ...
      <span className="text-sm font-semibold text-midnight">{value || '—'}</span>
    </div>
  );
}

export default function ParcelDetailsPanel({ parcel, exploitationName, isOpen, onClose }) {
  return createPortal(
    <AnimatePresence>
      {isOpen && parcel ? (
        <>
          ...
          <motion.aside
            ...
            className="fixed right-0 top-0 z-[9999] flex h-screen w-full max-w-md flex-col overflow-y-auto border-l border-iceBlue bg-arcticWhite shadow-[0_0_80px_rgba(2,48,71,0.18)]"
          >
            {/* Panel header */}
            ...
            <div className="flex-1 space-y-4 p-6">
              ...
              <div className="flex items-center justify-between gap-4 rounded-2xl bg-gradient-to-br from-oceanBlue to-aquaBlue p-5 text-white shadow-[0_12px_32px_rgba(0,119,182,0.22)]">
                <div>
                  ...
                  <p className="mt-1 text-4xl font-black">
                    ...
                    <span className="ml-1.5 text-xl font-semibold text-cyan-100">ha</span>
                  </p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <DetailRow
                  icon={Navigation}
                  label="Latitude"
                  value={parcel.latitude ? `${Number(parcel.latitude).toFixed(6)}°` : '—'}
                />
                <DetailRow
                  icon={Navigation}
                  label="Longitude"
                  value={parcel.longitude ? `${Number(parcel.longitude).toFixed(6)}°` : '—'}
                />
              </div>
